...
>
> **どこを探すかは、2つある**（2026-08-19 に片方の抜けを踏んだ）。
> **スレッド**は親ではなく**最後の動き**で窓を判定する——親の時刻で切ると、
> 「古いスレッドへの新しい返信」という**返信し忘れの本命**が一生拾えない。
> **チャンネル直下の名指し**も拾う——スレッドではないので、スレッドだけを見ていると
> 素通りする（実際 `@Bob …` の呼びかけを2時間半取りこぼした）。直下で拾うのは
> **名指しだけ**で、雑談は拾わない（通常の追従と同じ線引き）。
>
> **自分の id は起動時に確かめる**（`auth.test`）。以前はイベントの context からしか
> 取っておらず、**最初のイベントが届くまで自分が誰か分からなかった**。名指しの判定には
> 自分の id が要るので、**確認がいちばん要る再起動直後に、名指しの検出だけが死んでいた**。
> 分からないまま探した場合は**0件と言わずに理由を返す**——「探して0件」と「探せなかった」は
> 別物である。
...
> [!IMPORTANT] **決定（2026-08-24）: 留守明けは、前回止まった時点まで遡る（上限3日）。**
> 積み残しの窓（既定12時間）は「起動のたびに何日も前のスレッドへ返信し始める」のを防ぐため
> のもので、その判断は正しい。**壊れていたのは前提の方**——「基本的に動いている」を前提に
> していたが、実際には4日間止まり、その間に来た呼びかけは**一件も拾われなかった**。
>
> 「古い話を掘り返さない」と「**留守にしていた分に応える**」は別のことなので、別の数字で決める。
>
> | | |
> |---|---|
> | 前回いつまで動いていたか | **死活の記録**（#78）の最後の一打 |
> | 遡る上限 | 3日。超えて止まっていたなら、拾い直すより人に言われた方が自然 |
> | 遡るのはいつ | **起動直後の1回だけ**（動いている間は留守にしていない） |
>
> `agent.stopped` は使わない。**正常終了でしか残らず、困るのはたいてい強制終了の方**である。
> 死活の記録なら動いている間ずっと打っているので、どんな落ち方をしても最後の一打が残る。
>
> ただし打つ側と読む側の順序に注意が要る。記録は1行を上書きしていく形なので、
> **起動して打ってから読むと「たった今」しか返らない**（前回が消える）。打つ前に読む。
>
> 広げたときは黙らない（ログ・監査・イベント）。上限で打ち切ったことも残すので、
> **拾えなかった分があること**は後から言える。
