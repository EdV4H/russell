...
> [!IMPORTANT] **決定（2026-08-13）: 倒れたことは、運用者に届ける。**
> 黙ること自体は設計どおりだが、**黙ったことが誰にも届かないと、外からは4つが同じ姿になる**——
> 正常に静か / 監査が壊れて全停止 / 凍結状態が読めず沈黙 / プロセスが落ちた。
> コアはイベントを出していたが、**購読者が1人もいなかった**（#25）。
>
> `alerts` プラグインが安全系イベントを流す。宛先は通信面が提供し、
> 無い構成・未設定ではプロセスログにだけ出す。
>
> **流す先は管理チャンネルと分ける**（`RUSSELL_ALERT_CHANNEL`）。キルスイッチの発動記録は
> 「人がやった1回」だが、通知は**壊れている間ずっと出る**。同じ場所へ流すかは運用が決めることで、
> **既定は Slack へ流さない**（明示的に設定したときだけ）。
>
> **この経路だけは監査を通さない。** 通常の送信は「監査が残せないなら送らない」だが、
> ここは**その監査が壊れたことを伝える経路**なので、同じ規則を当てると
> 「壊れたときだけ黙る」という最悪の性質になる。代わりに範囲を狭くしてある:
> **宛先は管理チャンネル固定**、**本文は定型**（何が起きたか・道具名・件数だけ）、
> **会話の内容は一切含まない**。キルスイッチの発動記録が同じ経路で流れるのと同じ扱いである。
>
> **同じ知らせで埋めない。** 1件目はすぐ出し、以降は窓（既定10分）の間ためて件数付きで出す。
> **溜めた件数は捨てない**——「10分で1件」と「10分で400件」はまったく違う話なので。
