...
> [!IMPORTANT] **決定（2026-08-13）: 承認は原則を緩めるのではなく、人を1人挟む。**
> `external_*` / `irreversible_write` はこれまで**無条件で拒否**されていた（安全側の未実装）。
> 承認を入れたが、**通らないときは通らないまま**である:
>
> | | |
> |---|---|
> | 通信面が承認を持たない | **実行しない** |
> | 依頼の文脈が無い（自発的な実行など） | **聞かずに実行しない** |
> | 期限（既定10分）まで押されない | **実行しない** |
> | 承認の記録が残せない | **実行しない** |
>
> **待ち続けない**のは、返らない答えでターンが終わらなくなるから。諦めて通すのは論外なので、
> 「時間切れ＝却下」以外に取りようがない。
>
> **誰が押してよいかは通信面が決める。** Slack の user id を知っているのはあちらで、
> コアは「人に聞いた」までしか持たない。Slack 実装では**依頼した本人か運用者**だけが押せる
> （キルスイッチと同じ名簿）。押した人は監査に残す——**理由の本文は残さない**（人が自由に
> 書ける欄なので、何が入るか分からない, A1-5）。
>
> 押す前に**何をするのか・どの分類か・中身は何か**を見せる。「承認しますか？」だけのボタンは、
> 判断できないまま押させることになる。
