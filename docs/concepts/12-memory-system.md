...
> **引き受けた作業（`todos`）は記憶に持つ。** メモ帳は TTL で消え、索引カードは状態を
> 持たないので乗らない。忘却曲線は適用せず、代わりに「何日動いていないか」を出す。
> **タスク管理ツールにはしない**——個体が消化する作業だけを持ち、チームで共有するタスクは
> 装備（外部ツール）側に置く。境界は「誰が消化するか」。
> → [ADR 0009](../adr/0009-todos-are-commitments-not-tasks.md)

> [!IMPORTANT] **決定（2026-08-11）**
> **単語帳（ユビキタス言語）と個人カルテは、索引カード `entities` に置く。** `type` で分ける
> （`term` / `person`）。本棚と分けたのは**忘却の意味と引き方が違う**から——使わなくても
> 定義は忘れず、直近ではなく**その語が出た文**で引く。想起は別名の一致で、モデルを使わない。
> いま動いているのは `term` だけで、`person` は「何を書かないか」を決めてから。
> → [ADR 0008](../adr/0008-glossary-and-dossiers-are-index-cards.md)

> [!IMPORTANT] **決定（2026-08-11）**
> **機微情報は記憶から落とさない。印を付けて残し、公開経路（日記）で止める。**
> 落とすと仕事に使えなくなる——「知っているが言わない」のが同僚として自然な振る舞い。
> 検出は決定論の純関数で、**人物評価は捕まえられない**ことを明示してある。
> → [ADR 0007](../adr/0007-sensitive-guard-marks-not-blocks.md)

> [!IMPORTANT] **決定（2026-08-11）**
> **メモ帳と本棚は、材料の側で分かれる。** 本は会話中に直接書かず、夜間バッチが
> **3件以上のメモに現れた話題**を昇格させて作る（明示的に「覚えておいて」と頼まれた場合を除く）。
> 同じ1往復から両方を書かせると、時間の射程が違うはずの2つが同じ粒度になる。
