...
同じ `contextId` の**直近のやりとりを保持し、モデル呼び出しに `ModelRequest.history` として
渡す**。スレッドで「で、どうする？」と言われて話が通じるのはこれがあるから。

**これは記憶ではない**（[ADR 0002](../adr/0002-conversation-context-is-not-memory.md)）。
記憶システム（メモ帳・日記・本棚・書庫）は Bob が「保つ」ものの体系で、こちらは
「いま抱えている」もの。`contextId` が何を指すかは**通信面が決める**（Slack はスレッド、
DM はチャンネル、CLI はセッション）。コアは中身を解釈しない。

記憶と両方ないと会話にならない——記憶があっても直前の発言を忘れていたら話が通じないし、
直前しか覚えていなければ昨日の話ができない。

- 1文脈あたり直近 **20 発言**（user/assistant 合計）。超えたら古いものから捨てる
- プロセス内にのみ保持。**再起動で消える**（永続化と要約は長い会話を扱うときに §4 と接続する）
