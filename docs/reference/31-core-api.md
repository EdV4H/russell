...
同じ `contextId`（Slack ならスレッド、DM ならチャンネル）の**直近のやりとりを保持し、
モデル呼び出しに `ModelRequest.history` として渡す**。スレッドで「で、どうする？」と
言われて話が通じるのはこれがあるから。

**メモ帳・本棚（長期記憶）とは別物。** あちらは「後から思い出すために書き留めたもの」で、
書き留めなかったことは残らない。こちらは「いま話している最中に覚えている直前の数往復」。
人間で言えば前者が手帳、後者が短期記憶で、**両方ないと会話にならない**——記憶があっても
直前の発言を忘れていたら話が通じないし、直前しか覚えていなければ昨日の話ができない。
