...
> [!IMPORTANT]
> **決定（2026-07-22）: 本 suite を全装備/プラグイン共通の必須ゲート（v1）として採用。** 全項目が「合否二値の機能テスト」で、閾値サインオフを要さない。`terminal`（danger_level 3、§9.2）にはサンドボックスVM限定・全コマンド event_log・破壊系 HITL 必須の**装備固有補遺**を足す（下記 TODO）。CI 化は可能な項目から順次。
>
> [!TODO]
> **残（実装フェーズで確定）: 装備固有補遺の項目化と CI 自動化範囲。決定オーナー: 発注側技術責任者。** terminal の追加3項目を suite に明文化し、どこまでを CI で常時回すかを実装時に決める。共通項目の内容自体は v1 確定。
