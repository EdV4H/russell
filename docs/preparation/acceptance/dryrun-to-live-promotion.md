...
- [ ] dryrun 観測日数・最小母数の確定
- [ ] レビュアーの割り当て（下記 [!TODO]）
- [ ] 妥当率・誤検知率・有害件数の合格ラインの確定（[`test-strategy.md`](./test-strategy.md) と同期）
- [ ] レビュー判定の記録テンプレート（Finding ごとに 妥当/不要/有害 + コメント）
- [ ] 昇格・切り戻しの承認と event_log 記録の運用
- [ ] proactivity 週次引き上げの手順

> [!TODO]
> **レビュアーと合格閾値の確定。決定オーナー: ドッグフーディング推進者 + 発注側技術責任者。** (1) 誰が dryrun 出力をレビューするか（1名か複数か、複数なら合意方法）。(2) 観測日数（提案10営業日）・最小母数（提案20件）・妥当率（提案70%）・誤検知率（提案15%）・有害0件の確定。閾値は [`test-strategy.md`](./test-strategy.md) P3-1/P3-2 と必ず同一値にする（二重管理禁止）。(3) live 公開承認者（[`../operations/`](../operations/) の運用体制で定める承認者と一致させる）。
