...
> [!IMPORTANT]
> **決定（2026-07-22）: 段階発注（P0 → P1 → P2 → P3 順）を採用。** 各発注は [`../acceptance/test-strategy.md`](../acceptance/test-strategy.md) の当該フェーズバー通過を検収の入口とし、通過してから次フェーズを発注する。P0/P1 の結果次第で P3 を発注しない選択肢を残す（オプション契約推奨）。
>
> [!TODO]
> **残: 各発注の Go/No-Go 判断者の指名（人間）。決定オーナー: 発注責任者。** 誰が各フェーズ検収と次発注の Go/No-Go を判断するか（[`../operations/ownership-and-approval.md`](../operations/ownership-and-approval.md) の承認者と一致させる）。
...
- [x] 各フェーズ完了定義を v1 採用（上記の機能的条件 + test-strategy バー通過）
- [ ] 完了定義と検収・支払いの紐付けの確定（段階発注の各検収＝支払いトリガー）
...
> [!IMPORTANT]
> **決定（2026-07-22）: 上記の裁量／承認仕分けを v1 採用。** 「発注側承認要」の項目は下記コードレビュー体制（全 PR 発注側レビュー＋重点パス）で機械的に担保する。
>
> [!TODO]
> **残: 各「承認要」行の承認者（人事／法務／技術責任者のいずれか）の割り当て（人間）。決定オーナー: 発注責任者 + 技術責任者。** [`../operations/ownership-and-approval.md`](../operations/ownership-and-approval.md) の承認者ロールと一致させる。
...
### 方針（決定 2026-07-22: 全 PR を発注側レビュー）

- **すべての PR に発注側レビュアーの承認を必須とする**（main への直 push 禁止・外注内レビューだけでは通さない）。**← 確定。**
- そのうえで、**セキュリティ／労務直結パス**は発注側レビューでも特に重点確認（漏れると被害が大きい）:
...
- 全 PR 発注側レビューはレビュー負荷が高いので、**上記の重点パスは複数名 or 専任、それ以外は1名で軽量承認**という運用強弱をつけてよい（承認"必須"は全 PR、深さは可変）。
- セキュリティ観点は §12 のガードレール（信頼ラベル伝播・untrusted の特権ツール流入禁止）をチェックリスト化して適用する。

### チェックリスト

- [x] 方針確定: **全 PR に発注側レビュー承認を必須**（main 直 push 禁止）
- [ ] main 保護・PR 必須レビューの設定（[`../infra/setup-checklist.md`](../infra/setup-checklist.md) の CI 方針と連動）
- [ ] CODEOWNERS で全体レビュー＋重点パスの割り当てを機械強制
- [ ] セキュリティレビュー・チェックリストの整備

> [!TODO]
> **残: 発注側レビュアーの氏名・SLA・エスカレーション先の指名（人間）。決定オーナー: 技術責任者。** 全 PR 発注側レビューは確定。誰がレビュアーか（重点パス用の専任を置くか）、レビュー SLA（何営業日以内）、不在時エスカレーション先を確定する。CODEOWNERS で機械強制。
