...
| config 管理者 | ○ |
| 実装/運用担当（オンコール） | ○ |
| ドッグフーディング住人 | `/ryo stop`（個体単位）は○。全体停止は権限者へ即連絡 |
...
| レベル | 方法 | 範囲 | 効き方 |
|---|---|---|---|
| 1. 個体単位・通常経路 | Slack で `/ryo stop <個体名>` | その個体の全自発行動を凍結 | DB 経由。副作用の直前に再検査（§5.1） |
| 2. 全体・通常経路 | `/ryo stop --all` | 全個体を凍結 | DB 経由 |
...
```
発動
 → #ryo-管理 に自動で「キルスイッチ発動: <個体/全体> / 発動者 / 理由 / 時刻」を記録
