...
export const TOOL_DESCRIPTIONS: Record<string, string> = {
  ...
  "meeting.transcript":
    ...
    "終わった会議の議事録を探すのは drive.search（こちらは Drive の話）。入力は不要",
  "slack.post":
...
**この JSON を書くこと以外に、道具を呼ぶ手段はありません。** あなたの実行環境が
ツール呼び出しの仕組みを持っていても、そこに上の名前は存在しません——呼んでも
「そんな道具は無い」と返るだけで、**調べものは失敗します**（実際にそうなりました）。
道具を使うのは、この JSON を返したときだけです。
