...
export interface SourceResult<T = unknown> {
  ...
  data?: T;
  /**
   * うまくいかなかった理由の**手がかり**（HTTP のステータスなど）。
   *
   * **本文は入れない。** 外部の応答をそのまま入れると、機微情報が会話や監査へ流れる（A1-5）。
   * ここに入れてよいのは「404」「権限が足りません」のように、**それ自体が中身でないもの**だけ。
   *
   * 無くても動くが、無いと「読めませんでした」としか言えず、
   * **権限の問題なのか、そもそも存在しないのかが分からない**。
   */
  detail?: string;
}
